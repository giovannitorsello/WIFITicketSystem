...
/**
* Spry Accordion jMaki widget
* 
* @author Carla Mott
* @constructor
* @see http://wiki.java.net/bin/view/Projects/jMakiAccordionDataModel
* @see http://labs.adobe.com/technologies/spry/samples/
*/
jmaki.widgets.spry.accordion.Widget = function(wargs) {
    
    var _widget = this;    
    var gradient = "AccordionPanelTab";
    var rows = [];
    var selectedIndex = 0;
    var autoHeight = true;
    var counter = 0;
    ...
    function showModelDeprecation() {
        if (!showedModelWarning) {
            jmaki.log("Spry accordion widget with id " + wargs.uuid + " uses the incorrect data format. Please see " +
    ...
    if (wargs.args && wargs.args.gradient) {
        ...
        if(args.gradient) {
            var gradients = {
                "default":"AccordionPanelTab",
                "aqua":"AquaAccordionTab",
                "blue":"BlueAccordionTab",
                "green":"GreenAccordionTab",
                "gray":"GrayAccordionTab"
            };
            var o = gradients[args.gradient];
            gradient = (typeof o != 'undefined') ? o : "AccordionPanelTab";
        }
        if (wargs.autoHeight) {
