...
jmaki.namespace("jmaki.widgets.jquery.table");

/**
* jQuery UI Tablesorter Wrapper Widget
*
* @author Ahmad M. Zawawi <ahmad.zawawi@gmail.com>
* @constructor
* @see http://tablesorter.com/docs/
*/
jmaki.widgets.jquery.table.Widget = function(wargs) {

    var _widget = this;
    var uuid = wargs.uuid;
    var publish = "/jquery/table";
    var subscribe = ["/jquery/table", "/table"];
    var idMappings = {};
    var counter = 0;
    jmaki.debug = true;
    ...
    function doSubscribe(topic, handler) {
     ...
     function init() {
    	...
    	_table.node.id = uuid + "_table";
    	_table.node.className = "tablesorter";
    	_widget.container.appendChild(_table.node);
    	_widget.createHeader();
    	_table.tbody = document.createElement("tbody");
    	_table.node.appendChild(_table.tbody);
    	for (var i=0; _widget.model.rows && i < _widget.model.rows.length; i++) {
        	_widget.addRow(_widget.model.rows[i], _table.tbody);
        }
        $("#" + uuid + "_table").tablesorter();
    }
     
     this.createHeader = function() {
    	 _table.thead = _widget.createRow(_table.node, true);
    	 _table.node.appendChild(_table.thead);
    	var _row = _widget.createRow(_table.thead); 
 		for (var i=0; i < _widget.model.columns.length; i++) {  
			var contents = _widget.model.columns[i].label;  			
			if (contents ) {
				_widget.addCell(_row ,contents, true);
			}
		}
     }
    ...
    this.createRow = function(_parent, _isHeader) {
    	var target = _table.node;
    	if (typeof _parent != "undefined") {
    		target = _parent;
    	}
    	var isHeader = false;
    	if (typeof _isHeader == "boolean") {
    		isHeader = _isHeader;
    	}
        var _row;
/*        if (jmaki.MSIE) {
        	if (isHeader) {
        		_row = _table.node.createTh();
        	} else {
        ...
        */
        	if (isHeader) {
        		_row = document.createElement("thead");
        	} else {
    ...
    this.addCell = function(_row, _val, _th) {
        var _cell;
        var isHeader = false;
        if (typeof _th == "boolean" && _th == true) isHeader = true;
        if (jmaki.MSIE) {
          if (isHeader) _cell = _row.insertCell(-1);
          else _cell = _row.insertCell(-1);
        } else {
            if (isHeader) _cell = document.createElement("th");
            else _cell = document.createElement("td");
    ...
    this.addRow = function(_in, _targetNode) {
    	var _target = _table.node;
    	if (typeof _target != "undefined") _target = _targetNode;
    	var _row = _widget.createRow(_target);
    	_row.item = _in;
        jmaki.log("_row.author "+_in.author);
        jmaki.log("_row.id "+_in.id);
    	if (_widget.model.columns) {
    		for (var i=0; i < _widget.model.columns.length; i++) {
                    var contents = _in[_widget.model.columns[i].id];  			
                    if (contents ) {                            
    			_widget.addCell(_row,contents);
    		    } else {
                        _widget.addCell(_row,"");
                    }                    
    		}
    	}
		...
		if (idMappings[_lid]) {
			jmaki.log(wargs.uuid
							+ " : Warning. Attempt to add record to yahoo.dataTable with duplicate row id: "
							+ _lid + ". Autogenerating new id.");
			while (idMappings[_lid])
				_lid = genId();
		}
		idMappings[_lid] = _row;  
    }
    
    this.myaddRow = function(_in){
        var data = _in.value;
        _widget.addRow(data, _table.tbody);
    };
    ...
    this.select = function(b) {
      ...
      //  console.log("targetId "+b.targetId);
     //  console.log("index "+index);
        if (typeof index != 'undefined'){
      //      _widget.dataTable.unselectAllRows();
     // console.log("idMappings "+ idMappings[index]);
        	var rowId = idMappings[index];
        	jmaki.log("jids=" + jmaki.inspect(idMappings, 1));
      
      //TODO : change the row color:
      alert("index=" + index + " rowid=" + rowId);
    ...
    this.postLoad = function() {
       ...
       // track the subscribers so we can later remove them
        ...
        for (var _i=0; _i < subscribe.length; _i++) {
            ...
            doSubscribe(subscribe[_i]  + "/addRow", _widget.myaddRow);
